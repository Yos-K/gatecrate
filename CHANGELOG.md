...
### Added
- **R17: `out=` への発火条件混入を検出（es-lint-info）**: `in=` には出所検査(R6)があるのに、
  `out=` は「存在するか」しか見られていなかった。その非対称のため **`out=常時`** のような
  欄の取り違え——発火条件を出力欄に書く——が ERROR=0 で通過する。実消費者モデルで発生し、
  収束ループ(es-converge)が緑にした後も残っていた。
  しかも `when=` に「常時」が含まれると R11 は即 `continue` して `out` を一切見ないため、
  **この穴は R11 では塞げない**。
  設計上の注意: **out の選択肢が既存語彙に解決するかは要求しない**。判定結果は新しい語で
  表すのが自然であり（`out=受理|入力不正`）、解決を強制すると正当な用法を弾く——
  試作段階で既存モデル4本に40件の誤検出を出したため、発火条件語（常時/毎回/無条件/always/
  いつでも）の混入だけを検出する形に絞った。挙動テスト3性質。
