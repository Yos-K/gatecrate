...
N act_cust    actor    購入者 | role=書籍を選び注文を出す利用者。業務の起点（誰が何を求めるか）。
...
N agg_order   aggregate 注文 | fields=注文ID:一意キー(同一性キー); 顧客ID:利用者識別; 明細:書籍IDと数量の組 | states=受付 // 注文を受け取り、成立がまだ確定していない局面|確定 // 注文として成立した終端|取消 // 成立せず打ち消された終端 | transitions=確定する:受付->確定 // 決済成立を受けて注文を確定させる; 取消する:受付->取消 // 在庫切れ・決済失敗時に注文を打ち消す | invariant=明細は1件以上 かつ 数量は正 | role=1件の注文の明細と進行状態を持つ実体。
