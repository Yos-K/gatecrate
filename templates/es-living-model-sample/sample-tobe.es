...
N agg_order   aggregate 注文 | fields=注文ID:一意キー(同一性キー); 顧客ID:利用者識別; 明細:書籍IDと数量の組 | states=受付|確定|取消 | transitions=確定する:受付->確定; 取消する:受付->取消 | invariant=明細は1件以上 かつ 数量は正 | role=1件の注文の明細と進行状態を持つ実体。
...
N agg_pay     aggregate 決済 | fields=決済ID:一意キー(同一性キー); 金額:通貨額 | states=受信|与信済|売上確定|返金済 | transitions=与信する:受信->与信済|与信失敗; 売上確定する:与信済->売上確定; 返金する:与信済->返金済 | invariant=遷移は定義された矢印のみ かつ 決済IDで冪等(何度実行しても結果が同じ) | role=1回の決済の進行状態(受信→与信→売上確定/返金)を持ち、二重課金と順序違反を防ぐ中核。
