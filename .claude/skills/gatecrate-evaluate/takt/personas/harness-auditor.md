...
- Evidence comes from `collect-gate-history.sh` (firing/cost), `probe-gate-liveness.sh` (survival
  proof), `classify-gate-type.sh` (mechanical gate type — do NOT hand-classify detection vs
  prevention), and `git log` (axis-2 churn). Prevention gates fire ZERO times when they work;
  detection gates earn their keep by catching real defects. The two are judged differently.
- **The axis-1 verdict is already mechanized**: pipe the firing TSV through `gate-roi-verdict.sh`
  (it routes by type — prevention fires=0 → keep, costly cold detection → removal-candidate, etc.).
  Do NOT re-derive it. Your job is the conditions it flags to a human: ② zero-uniqueness on a
  removal-candidate, prevention's risk-gone, the advisory "is the signal consumed?", and all of
  axis 2. Verify the hypothesis against evidence; never rubber-stamp it.
